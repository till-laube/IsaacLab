...
"""Custom environment for single UR5e manipulation with controller visualization."""
...
from .ur5e_single_manipulation_env_cfg import Ur5eSingleManipulationEnvCfg


class Ur5eSingleManipulationEnv(ManagerBasedRLEnv):
    """Custom environment for single UR5e manipulation with controller visualization.
    ...
    cfg: Ur5eSingleManipulationEnvCfg
    ...
    def _create_controller_visualization(self):
        ...
        try:
            # Controller frame visualization
            controller_marker_cfg = VisualizationMarkersCfg(
                prim_path="/Visuals/ControllerFrame",
                markers={
                    "frame": sim_utils.UsdFileCfg(
                        usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/UIElements/frame_prim.usd",
                        scale=(0.15, 0.15, 0.15),  # Larger than TCP frame (0.1) for distinction
                    ),
                }
            )
    ...
    def _update_controller_visualization(self):
        ...
        try:
            ...
            tcp_pos_w = self.scene["control_frame"].data.target_pos_w[0, 0, :]  # [3]
            tcp_quat_w = self.scene["control_frame"].data.target_quat_w[0, 0, :]  # [4] as [w, x, y, z]
        except Exception as e:
        ...
        if controller_pose is not None:
            ...
            if self._first_viz_update:
                print(f"[Controller Viz] ✓ Successfully visualizing controller pose!")
                ...
                print(f"[Controller Viz]   Compare it with the TCP frame at the gripper")
